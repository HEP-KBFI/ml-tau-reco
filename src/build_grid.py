    ...
    def processJets(self, data):
        ...
        for part in self.parttype:
            ...
            for cone in ['inner_grid', 'outer_grid']:
                grid = CellGrid(self.cfg[cone]['n_cells'], self.cfg[cone]['n_cells'], self.cfg[cone]['cell_size'], self.cfg[cone]['cell_size'])
                if cone == 'inner_grid':
                ...
                assert(list_part_info_alljet.shape[0] == pow(self.cfg[cone]['n_cells'], 2) * len(part_var_list[part]) * len(reco_tau_p4))
        return write_info
