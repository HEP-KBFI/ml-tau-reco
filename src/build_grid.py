...
class GridBuilder(BasicTauBuilder):
    ...
    def build_p4(self, part=''):
        if part:
            return vector.awk(
            ak.zip({"px": self.data[part].x, "py": self.data[part].y, "pz": self.data[part].z, "mass": self.data[part].tau})
        )
        return vector.awk(
            ak.zip({"px": self.data.event_reco_cand_p4s.x, "py": self.data.event_reco_cand_p4s.y, "pz": self.data.event_reco_cand_p4s.z, "mass": self.data.event_reco_cand_p4s.tau})
        )
    ...
    def process_onejet(self, etas, phis, inner):
        ...
        if self.reco_tau_p4.pt[self.jetidx] == 0: return list_part_var
        ...
        for idx, (eta, phi) in enumerate(zip(etas, phis)):
            ...
            if (self.filledgrid[etaidx,phiidx]) >= 2: continue
            filledpartidx = self.filledgrid[etaidx,phiidx]
            offset = filledpartidx*self.max_var - 1
            list_part_var[offset+Var.rel_pt.value][etaidx,phiidx] = (self.pt_sorted_cand_pt[self.pt_sorted_cone_mask>0][self.jetidx][idx]) / self.reco_tau_p4.pt[self.jetidx]
            list_part_var[offset+Var.dtheta.value][etaidx,phiidx] = self.pt_sorted_cand_dtheta[self.pt_sorted_cone_mask>0][self.jetidx][idx]
            list_part_var[offset+Var.dphi.value][etaidx,phiidx] = self.pt_sorted_cand_dphi[self.pt_sorted_cone_mask>0][self.jetidx][idx]
            list_part_var[offset+Var.mass.value][etaidx,phiidx] = self.pt_sorted_cand_mass[self.pt_sorted_cone_mask>0][self.jetidx][idx]
            list_part_var[offset+Var.pdgid.value][etaidx,phiidx] = self.pt_sorted_cand_pdgid[self.pt_sorted_cone_mask>0][self.jetidx][idx]
    ...
    def calculate_dangle(self):
        return np.sqrt(self.pt_sorted_cand_dtheta * self.pt_sorted_cand_dtheta + self.pt_sorted_cand_dphi * self.pt_sorted_cand_dphi)
    ...
    def get_cone_mask(self, inner_grid):
        if inner_grid:
            return ak.where(self.pt_sorted_cand_dangle < self.signalcone, ak.ones_like(self.pt_sorted_cand_dangle), ak.zeros_like(self.pt_sorted_cand_dangle))
        else:
            mask_outside_signal = ak.where(self.pt_sorted_cand_dangle > self.signalcone, ak.ones_like(self.pt_sorted_cand_dangle), ak.zeros_like(self.pt_sorted_cand_dangle))
            mask_inside_isolation = ak.where(self.pt_sorted_cand_dangle < 0.5, ak.ones_like(self.pt_sorted_cand_dangle), ak.zeros_like(self.pt_sorted_cand_dangle))
            total_mask = mask_outside_signal + mask_inside_isolation
            total_mask = ak.where(total_mask==2, ak.ones_like(self.pt_sorted_cand_dangle), ak.zeros_like(self.pt_sorted_cand_dangle))
            return total_mask
    ...
    def processJets(self, data):
        ...
        for cone in ["inner_grid", "outer_grid"]:
            ...
            self.maskdeta, self.maskdphi = self.pt_sorted_cand_dtheta[self.pt_sorted_cone_mask>0], self.pt_sorted_cand_dphi[self.pt_sorted_cone_mask>0]
            self.bin_idx_eta = ak.unflatten(np.searchsorted(self.bins_eta, ak.flatten(self.maskdeta)), ak.count(self.maskdeta, axis=-1), axis=-1)
            self.bin_idx_phi = ak.unflatten(np.searchsorted(self.bins_phi, ak.flatten(self.maskdphi)), ak.count(self.maskdphi, axis=-1), axis=-1)
            ...
            if self.do_plot: self.plot(cone)
