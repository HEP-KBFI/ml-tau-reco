...
class StripAlgo:
  ...
  def updateStripP4(strip):
    strip.p4 = vector(pt = 0., phi = 0., theta = 0., mass = 0.)
    ...
    for seedCand in seedCands:
      if not seedCand.barcode in seedCandFlags:
        ...
        while stripBuildIterations < self.maxStripBuildIterations or self.maxStripBuildIterations == -1:
          isCandAdded = False
          isCandAdded |= addCandsToStrip(currentStrip, seedCands, seedCandBarcodesPreviousStrips, seedCandBarcodesCurrentStrip)
          isCandAdded |= addCandsToStrip(currentStrip, addCands, addCandBarcodesPreviousStrips, addCandBarcodesCurrentStrip)
