...
def buildParticleTransformerTensors(jet_p4, 
                                    jet_constituent_p4s, 
                                    jet_constituent_pdgIds, jet_constituent_qs,
                                    jet_constituent_d0s, jet_constituent_d0errs, jet_constituent_dzs, jet_constituent_dzerrs,
                                    metric_dR_or_angle, metric_dEta_or_dTheta, 
                                    max_cands):
    ...
    for idx in range(num_jet_constituents):
        ...
        part_isChargedHadron      = 1. if jet_constituent_abs_pdgId == 211 else 0.
        part_isNeutralHadron      = 1. if jet_constituent_abs_pdgId in [130, 2112] else 0.
        part_isPhoton             = 1. if jet_constituent_abs_pdgId == 22 else 0. 
        part_isElectron           = 1. if jet_constituent_abs_pdgId == 11 else 0. 
        part_isMuon               = 1. if jet_constituent_abs_pdgId == 13 else 0. 
        if abs(part_charge) > 0.5 and part_d0 > -99. and part_dz > -99.:
...
class ParticleTransformerDataset(Dataset):
        ...
        for file in filelist:
            ...
            for idx in range(num_jets_in_file):
                ...
                v_tensor, x_tensor, node_mask_tensor = buildParticleTransformerTensors(
                  ...
                  jet_constituent_pdgIds, jet_constituent_qs,
                  jet_constituent_d0s, jet_constituent_d0errs, jet_constituent_dzs, jet_constituent_dzerrs,
                  self.metric_dR, self.metric_Eta, 
                  self.max_cands)
    ...
    def __getitem__(self, idx):
        if idx < self.num_jets:
            return {"v": self.v_tensors[idx], "x": self.x_tensors[idx], "mask": self.node_mask_tensors[idx]}, self.y_tensors[idx]
        else:
