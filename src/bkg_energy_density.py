...
def p4_to_pseudojet(mc_p4):
    ...
    for i in range(len(mc_p4_px)):
        ...
        for j in range(len(mc_p4_px[i])):
            pseudojet_particle = fastjet.PseudoJet(mc_p4_px[i][j], mc_p4_py[i][j], mc_p4_pz[i][j], mc_p4_E[i][j])
...
def compare_different_environments(cfg: DictConfig) -> None:
    ee_mc_p4 = load_ee_file_contents(cfg.samples.ee, max_events=cfg.max_events)[0]
    pp_mc_p4_wPU = load_pp_file_contents(cfg.samples.pp, branch="PFParticlesAll", max_events=cfg.max_events)[0]
    ...
    info_dict = {
        "ee": {"rho": ee_rhos, "sigma": ee_sigmas},
        "pp_wPU": {"rho": pp_wPU_rhos, "sigma": pp_wPU_sigmas},
        "pp_noPU": {"rho": pp_noPU_rhos, "sigma": pp_noPU_sigmas},
    }
