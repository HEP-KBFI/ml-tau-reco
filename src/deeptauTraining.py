...
def model_loop(model, ds_loader, optimizer, loss_fn, scheduler, is_train, dev):
    ...
    return loss_cls_tot * 100.0 / njets, 1, (class_true, class_pred), acc
...
def main(cfg):
    ...
    files_train = ["/local/laurits/CLIC_taus_20230215_v1/HPS/ZH_Htautau/reco_p8_ee_ZH_Htautau_ecm380_360.parquet"]
    files_val = ["/local/laurits/CLIC_taus_20230215_v1/HPS/QCD/reco_p8_ee_qcd_ecm365_1.parquet"]
    ...
    for iepoch in range(cfg.DeepTau_training.epochs):
        loss_cls_train, loss_p4_train, _, acc_cls_train = model_loop(
            model, ds_train_loader, optimizer, loss_fn, scheduler, True, dev
        )
        loss_true.append(loss_cls_train)
        acc_true.append(acc_cls_train)
        loss_cls_val, loss_p4_val, retvals, acc_cls_val = model_loop(
            model, ds_val_loader, optimizer, loss_fn, scheduler, False, dev
        )
